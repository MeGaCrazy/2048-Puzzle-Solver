...
def Minimax(grid, depth, start):
    ...
    for newgrid in newgrids:
        ret = min(ret, Maxmini(grid=newgrid, depth=depth - 1, start=start))
    return ret


def Maxmini(grid, depth, start):
    if Done(grid) or depth == 0 or (time.clock() - start) > 0.04:
        return heuristic(grid)

    ret = -2000000000
    for newgrid in get_all_possible_grid(grid):
        ret = max(ret, Minimax(grid=newgrid, depth=depth - 1, start=start))
    return ret
...
def solve(grid):
    dep = 4
    st = time.clock()
    return Minimax(grid=grid, depth=dep, start=st)
